  ...
  <div class="list_order">
    <div class="item_order" v-for="(o, i) in list" :key="i">
      ...
      <div class="goods_list" v-for="(obj, idx) in o.list" :key="idx">
        <div @click="go_order_detail(obj)" class="item_goods flex_sbc">
          <div class="img_block">
          ...
          <div class="right_info">
            <div class="price" v-if="!!o.integral">{{ o.integral }}积分</div>
            <div class="price" v-else>￥{{ obj[vm.price] }}</div>
            <div class="num">×{{ obj[vm.num] }}</div>
          </div>
        </div>
      </div>
      <div class="sum_price flex_sbc">
        <span v-if="!!o.integral">实付：{{ o.integral_total }}积分</span>
        <span v-else>实付：￥{{ o.sum_price.toFixed(2) }}</span>
      ...
      this.$refs.RefDataForm.validate(valid => {
        if (valid) {
          ...
          if(beforeListOneMsg.list.length > 1){
            ...
            queryDatas.buy_type = beforeListOneMsg.list[0].buy_type ? beforeListOneMsg.list[0].buy_type : 1;
		            queryDatas.integral = beforeListOneMsg.list[0].integral;
