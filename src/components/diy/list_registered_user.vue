	...
	<div class="diy_home diy_list diy_registered_user" id="diy_registered_user_list">
		...
		<div class="diy_view_table">
			<table class="diy_table">
				<tr class="diy_row">
								<th class="diy_title" v-if="$check_field('get','real_name','/registered_user/list')">
					真实姓名
				</th>
															<th class="diy_title" v-if="$check_field('get','user_label','/registered_user/list')">
					用户标签
				</th>
														<th class="diy_title" v-if="$check_field('get','user_photos','/registered_user/list')">
					用户照片
				</th>
								</tr>
								<tr class="diy_row" v-for="(o,i) in list" :key="o+i" @click="to_detail(o)">
													<td class="diy_field diy_text" v-if="$check_field('get','real_name','/registered_user/list')">
						<span>
							{{ o["real_name"] }}
						</span>
					</td>
																<td class="diy_field diy_text" v-if="$check_field('get','user_label','/registered_user/list')">
						<span>
							{{ o["user_label"] }}
						</span>
					</td>
															<td class="diy_field" v-if="$check_field('get','user_photos','/registered_user/list')">
						<img class="diy_img" :src="$fullUrl(o['user_photos'])" />
					</td>
								</tr>
			</table>
		</div>
		<!-- 轮播 -->
		 ...
		 <div class="carousel ins_s">
                <div class="slider" ref="slider">
                    ...
                    <router-link :to="'/registered_user/details?registered_user_id=' + o['registered_user_id']" class="lis_cont">
						                        <div class="diy_list_img_box" v-if="imgList.length" >
    					        <div class="diy_row" v-for="(item, index) in imgList" :key="item + index" v-show="$check_field('get',item.name,'/registered_user/details') && +item.is_img_list">
    						<div class="diy_title diy_list_img_title">
...
<script>
	export default {
			...
			return {
				...
				totalItems: 3,
						...
						itemList: [
								{
									title: "用户会员卡号",
									name: "user_membership_card_number",
									type: "文本",
										units: "",
										is_img_list: "0"
								},
		...
		methods: {
			...
			to_detail(v){
				this.$router.push("/registered_user/details?registered_user_id="+v.registered_user_id)
			},
