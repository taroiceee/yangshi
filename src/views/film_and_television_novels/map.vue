...
export default {
  ...
  methods: {
    async get_list() {
      const res = await this.$get("~/api/film_and_television_novels/get_list", {
        page: 1,
        size: 9999,
      });
      ...
      marker.addEventListener("click", function (e) {
        const content = `
              <div class="map_tips" style="height: 200px; overflow-y: auto;padding-bottom: 20px;">
                            	              		              						专辑名称: `+data.album_name+`<br>
              		              	                            	              		              						专辑封面: <img src="`+self.$fullUrl(data.album_cover)+`" style="width: 100%;"><br>
              		              	                            	              		              						作者姓名: `+data.authors_name+`<br>
              		              	                            	                            </div>
            `;
