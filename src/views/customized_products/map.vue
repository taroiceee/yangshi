...
export default {
  ...
  methods: {
    async get_list() {
      const res = await this.$get("~/api/customized_products/get_list", {
        page: 1,
        size: 9999,
      });
      ...
      marker.addEventListener("click", function (e) {
        const content = `
              <div class="map_tips" style="height: 200px; overflow-y: auto;padding-bottom: 20px;">
                            	                            	              		              						商品名称: `+data.name_of_commodity+`<br>
              		              	                            	              		              						分类名称: `+data.category_name+`<br>
              		              	                            	              		              						商品封面: <img src="`+self.$fullUrl(data.commodity_cover)+`" style="width: 100%;"><br>
              		              	                            	                            	              		              						定制库存: `+data.custom_inventory+`<br>
              		              	                            	                            	                            	                            	                            	                            	                            	                            	                            	                            </div>
            `;
